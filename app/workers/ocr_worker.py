...
celery = create_celery_app(settings, WorkerKind.OCR)


@celery.task(bind=True, name=PipelineTask.OCR_PROCESS)
def process_document(task, payload: dict[str, str]) -> None:
    """Run one durable OCR job in the isolated OCR worker pool."""
    validated_payload = IngestionJobPayload.model_validate(payload)
    worker_id = str(task.request.hostname or "ocr-worker")[:255]
    try:
        asyncio.run(run_ocr_job(validated_payload, worker_id))
...
async def run_ocr_job(payload: IngestionJobPayload, worker_id: str) -> None:
    ...
    service = OcrIngestionService(database, storage, settings, dispatcher)
    try:
...
def main() -> None:
    """Start the OCR pool consuming only its configured queue."""
